 ...
 */
 ...
 class ModernImageEditor {
    ...
    updateToolbar(activeTool) {
        ...
        // 显示当前工具的选项面板
        var currentPanel = document.getElementById(activeTool + "Options");
        if (currentPanel) {
    ...
    isPointInElement(x, y, element) {
        switch(element.type) {
            ...
            case "number":
                ...
                return x >= element.x - radius && x <= element.x + radius &&
                       y >= element.y - radius && y <= element.y + radius;
                       
            case "shape":
            case "highlight":
                return x >= Math.min(element.x, element.x + element.width) &&
                       x <= Math.max(element.x, element.x + element.width) &&
                       y >= Math.min(element.y, element.y + element.height) &&
                       y <= Math.max(element.y, element.y + element.height);
                       
            case "brush":
                if (element.points && element.points.length > 0) {
                    var minX = Math.min(...element.points.map(p => p.x));
                    var maxX = Math.max(...element.points.map(p => p.x));
                    var minY = Math.min(...element.points.map(p => p.y));
                    var maxY = Math.max(...element.points.map(p => p.y));
            ...
            <div class="property-item">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 5px;">
                    <div>
                        ...
                        <input type="number" class="property-input" id="propX" value="${Math.round(bounds.x)}" style="width: 100%;" title="X坐标">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: #6c757d;">Y坐标</label>
                        <input type="number" class="property-input" id="propY" value="${Math.round(bounds.y)}" style="width: 100%;" title="Y坐标">
                    </div>
                </div>
            </div>
            <div class="property-item">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-top: 5px;">
                    <div>
                        ...
                        <input type="number" class="property-input" id="propWidth" value="${Math.round(bounds.width)}" style="width: 100%;" title="宽度">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: #6c757d;">高度</label>
                        <input type="number" class="property-input" id="propHeight" value="${Math.round(bounds.height)}" style="width: 100%;" title="高度">
                    </div>
                </div>
            </div>
        `;
